...
export const PokemonList = ({ totalPokemonList, minimumRange, setMinimunRange }) => {
  ...
  let remainingPokemonQuantity = useRef(
    totalPokemonList.length - AMOUNT_POKEMON_SHOW
  );
  ...
  const showNextSheet = () => {
    ...
    remainingPokemonQuantity.current =
      remainingPokemonQuantity.current - AMOUNT_POKEMON_SHOW;
  ...
  useEffect(() => {
    const newPokemonSlice = totalPokemonList.slice(
      minimumRange,
      AMOUNT_POKEMON_SHOW + minimumRange
    );
  ...
  return (
    <>
      <div className={styles.boxButtons}>
        {minimumRange > 0 && (
          <button onClick={showPreviousSheet}>Anterior</button>
        )}
        {remainingPokemonQuantity.current > 0 && (
          <button onClick={showNextSheet}>Siguiente</button>
        )}
      </div>
      <div className={styles.boxPokemonlist}>
        {pokemonSlice &&
          pokemonSlice.map((pokemon, index) => {
            return <PokemonCard key={index} namePokemon={pokemon.name} />;
          })}
      </div>
      <div className={styles.boxButtons}>
        {minimumRange > 0 && (
